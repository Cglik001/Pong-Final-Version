...
let maxPoints = 3; //array of [3,5,10] for each i create drowpdown element
let scoreOne = 0;
let scoreTwo = 0;
...
function update(){
  ballBounce();
  paddleCollision();
  console.log("fora do if", gameOver)
  if(!gameOver){
    console.log("window")
...
function displayScoreTwo(){
  context.font = "48px Arial Bold "
  context.fillStyle = "#fff"
  context.fillText(scoreTwo, canvas.width/2 + 32, 50)
}
...
//const playerWinner = document.getElementById("insertplayerwinner")
...
function gameWinner() {
  if(scoreOne === maxPoints) {
    console.log("alert winner")
     ...
     //playerWinner.InnerText = "player One Wins!"
      gameOver = true;
      resetGame();
       //gameOver()
       ...
       //playerWinner.innerText= "player Two Wins!"
       gameOver = true;
       resetGame();
      // gameOver()
    }
